...
\begin{figure}
	...
	\end{center}
	\caption[Left: An example input image. Right: The corresponding post-processed crop which contains some misclassified noise.]{Left: An example input image. Right: The corresponding post-processed crop which contains some misclassified noise. A border has been added for clarity.}
...
Triplet Ranking Loss aims to avoid this issue by training on triplets of data points rather than pairs, with each triplet containing an Anchor, a Positive, and a Negative. SNNs which make use of Triplet Ranking Loss are sometimes named Triplet Networks in the literature \cite{hoffer_deep_2018}, however the only difference between the structure of an SNN using Pairwise Ranking Loss or Triplet Ranking Loss is the number of branches -- two or three respectively. 
...
In Chapter \ref{ch:SNNEvaluation}, the robustness of SNNs for the task of automatic most likely catalogue matching is evaluated. The effect of combining the NDD and AU SMRU data on model performance is explored, as well as the effect of background noise on embedding generation -- justifying the use of instance segmentation over bounding box detection before matching. Next, the generalisation of the approach is examined through the use of a second, smaller, real life photo-id catalogue. The problem of most likely catalogue matching is then framed as a standard image classification task, comparing against the developed approach. Comparisons between the system developed in this thesis and an existing photo-id aid are also presented. 
