...
Once an SNN capable of most likely catalogue matching was created, experimentation was performed to determine the effect of different input image variations.
...
\subsubsection{Individual Only Top-N Accuracy}\label{ch:ID,sec:EffectOfInputImageVariation,subsec:perSide,sub:individualOnlyTopN}
...
\subsection{Identification Without Noise}\label{ch:ID,sec:EffectOfInputImageVariation,subsec:NoNoise}

% Experimentation to show the effect noise has on the model's ability. Show comparison chart between models trained with noise and those trained without. In majority of cases the exlcusion of a noise class has hindered top-10 model performance. 
% try to modify chart in NDD-Detections-Model-Selection notebook to show top-1,5,10 accuracies stacked for noise vs no-noise comparison
