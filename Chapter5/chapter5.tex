...
%TODO: How does the best model meet the requirements? Add in top-1 confusion matrix for best model? If it struggles with class 19 highlight this alongside the KNN/prototypes uncatalogued individuals section?
...
\begin{table}[!t]
	...
	\resizebox{!}{.55\height}{
		...
		\begin{tabular}{ccccccccccccc}
			...
			\bottomrule                                                      
	\end{tabular}}
	\caption{Optimal SNN hyperparameters for each architecture-augmentation combination located using Bayesian optimisation over 100 iterations. Results given to 3 decimal places where applicable.}
...
To decide on a threshold value for the NDD AU SMURU dataset, each image not utilised for SNN training was ran through the model and the distances between it and the class prototypes were examined. Through this, a threshold value of 10 was determined - if the distances to all class prototypes was above this value, then it is likely that the individual may be uncatalogued. 

The use of class prototypes proved especially useful for determining noise. Through experimentation it was found that an image could be flagged as belonging to the noise class simply if the distance between the input image's embedding and the noise class prototype was the smallest, as seen in Figure XXX. This provides evidence to suggest that the model is capable of clustering examples of noise together well in the latent space, especially impressive as this class contains the highest intra-class variation as a result of containing all images of erroneous non-fin detections.
...
During experimentation, it was found that setting $K = 10$ alongside an uncertainty threshold of 30\% allowed for the best chance of flagging uncatalogued individuals. If no class reached above 30\% certainty, this was flagged to the user. This thresholding via KNN again worked well for warning the user the input may have been erroneously detected noise, as seen in Figure XXX. 
...
%TODO: Add in images showing input and output of the SNN. Maybe 1 image of an uncatalogued fin, 1 of a catalogued fin, and 1 of noise?  

 
% Sec: SNN Evaluation
% Limitations (uncatalogued individuals may just be changed - at least we're flagging and having a human decide. re-training needed for each catalogue.)
