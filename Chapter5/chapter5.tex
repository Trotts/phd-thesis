...
% Discuss need for prototypes at inference time, allows for generating of new classes without need for retraining
...
% Double check on VM to make sure TPE is the one used.



% Discuss how the optimisation results in final optimal values which are unlikely to be chosen by a human during a grid search. By allowing optimisation to occur over a continuous rather than discrete range, this also has the effect of locating optimal hyperparameters with values unlikely to be defined by humans for a grid search.  

\subsection{Data Augmentation Strategy}\label{ch:ID,sec:SNNDevelopment,sub:DataAugmentation}


% Discuss hyperparameter tuning using Optuna, show best model finding. 
% Discuss data augmentation strategies, all using best model hyperparameters found, show best strategy.
