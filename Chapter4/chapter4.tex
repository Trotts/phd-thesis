...
%TODO: Add in image format, resolution, camera settings
...
The data contained within NDD20 are large scale panoramic images, similar to the Zanzibar data before post-processing. As a result, there is a large amount of noise present in the images which should be ignored and removed before the dataset is utilised for individual identification. Whilst Chapter \ref{ch:ID} will focus on both the theory behind, and implementation of, a model capable of individual identification, this Section will detail the processing of NDD20 into a form usable for the task of individual identification. 

\subsection{Application of Post-Processing Techniques}\label{ch:NDD,sec:postProcessingNDD20,sub:applyingPostProcessing}

Utilising the post-processing techniques as outlined in Section \ref{ch:cetDet,sec:postProcessing}, the masks detected by model 20190902T0946 were used to create a dataset of images ready to train a model capable of individual identification. This dataset, known as Cropped Detections NDD20, is a collection of images which contain only the crop of the detected mask. Example images from Cropped Detections NDD20 can be seen in Figure XXX.

%TODO: show examples of the Cropped NDD20. Tiled, with class names.

Once images of the cropped masks had been created, these were then processed further into a folder structure which allowed for easy training of a model capable of individual identification. To facilitate this, each cropped mask was checked against the ground truth masks for the image. Any cropped mask images which contained a dorsal fin with identifying information in was placed in a directory containing other examples of that individual. Any fins that did not contain identifying information were removed as their identity could not be guaranteed. Any noise which had passed through the mask post-processing were included in a \texttt{noise} directory with the goal of allowing any future model to learn how to identify erroneous detections which have made it through processing. At this point, the Cropped Detections NDD20 dataset contained 39 classes including \texttt{noise}. Individuals 5, 6, 17, 27, 40, and 41 were removed by the post-processing due to poor quality detections. Upon examination few examples of these individuals were present in NDD20 and those that were showed the individuals in challenging positions for the detector such as at an extreme angle or with large amounts of splash.

As Cropped Detections NDD20 was produced from the output of the Mask-RCNN detector, example images for each individual are not perfect. If the detector has missed a part of the animal's dorsal fin for example, then this information will not be present in Cropped Detections NDD20. In order to quantify the effect this loss has, a second dataset called Cropped Ground Truths NDD20 was created. Images in this dataset were generated using the ground truth pixel masks present in NDD20 to crop rather than using the predicted masks generated by the detector. This creates a dataset representing perfect detections, as if the model's mAP@IOU[1.0] = 1.0. As a result this dataset does not contain a noise class, however does contain examples of all individuals. 


% Addition of AU_SMRU data
% Distribution of classes and num examples graph/table?



% Chapter Summary
% Creation of a dataset. Shown the model created previously is invariant to changes in geo, time, species. Baseline results for below water. 
% Processed into a form ready for individual ID.
