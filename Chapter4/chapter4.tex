...
In total, Segmented NDD20 contains 1243 images representing 43 classes including \texttt{noise}. Individuals \texttt{6} and \texttt{27} were removed from the dataset by the post-processing algorithm. Upon examination, images of these individuals were captured from extreme angles or with large amounts of splash, making it difficult for the detector to produce accurate segmentations. The dataset is highly skewed, as seen in Figure \ref{fig:segmented-ndd20-dist}, with approximately 66\% of images in the dataset labelled as \texttt{noise}. Classes representing individual animals contain a non-uniform number of example images varying between 33 examples for individual \texttt{11} and just one for individuals \texttt{5} and \texttt{35}. As such this dataset represents not just a fine-grained but also a few-shot problem, a combination which is extremely challenging for current computer vision methods. 

\begin{figure}
	\begin{center}
		\includegraphics[scale=0.5]{Chapter4/figs/seg-ndd20-dist.png}
	\end{center}
	\caption{The \texttt{ID} class label distribution for Segmented NDD20.}
	\label{fig:segmented-ndd20-dist}
\end{figure}

\section{Aberdeen \& St. Andrews University Additional Data}\label{ch:NDD,sec:NDD_AU_SMRU}

After post-processing the data collected during fieldwork into the Segmented NDD20 dataset, it was clear from the class distribution present (seen in Figure \ref{fig:segmented-ndd20-dist}) that it may be beneficial to increase the number of examples per individual, providing a photo-id model more data to learn from. As it was not possible due to sea state conditions to continue data collection in the Coquet to St. Mary's MCZ after 10/10/2019, it was possible for more bottlenose dolphin data to be obtained from external sources. 

% Aberdeen, St Andrews provided data to help aliviate. 2003-2019. 1827 photos of quality 3 (highest on scale used by Aberdeen). Each photo has 1 BND only. Work undertaken to match individuals between the two catalogues (23 inidvidual overlap between the two study areas, checked by us and Aberdeen). Show count of NDD20_AU_SMRU. 
